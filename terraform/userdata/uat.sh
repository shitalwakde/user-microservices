...
export NODE_ENV=uat
aws ssm get-parameters-by-path --path "/uat/user-service/" --with-decryption --region us-east-1 | jq -r '.Parameters[] | "export \(.Name | split("/") | last | ascii_upcase)=\(.Value)"' >> /home/ubuntu/.env
